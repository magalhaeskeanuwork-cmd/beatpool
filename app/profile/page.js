...
export default function Profile() {
  ...
  async function fetchProfile() {
    ...
    if (profileData?.role === 'artist') {
      const { data: requestsData } = await supabase
        .from('requests').select('*').eq('artist_id', user.id)
  ...
  async function handleUploadFullBeat(dealId, e) {
    ...
    await supabase.from('deals').update({ full_beat_url: publicUrl, status: 'completed' }).eq('id', dealId)
    fetchProfile()
  }
  ...
  function totalEarnings() {
    return deals
      ...
      .reduce((sum, d) => sum + Math.round(d.agreed_price * 0.80), 0)
  }
          ...
          <div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-[10px] font-mono uppercase tracking-[0.3em] text-white/30 mt-2 capitalize">{profile?.role}</p>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              {requests.filter(r => r.status === 'open').length === 0 ? (
                <div className="border border-white/5 p-12 text-center">
                  ...
                  <Link href="/request/new" className="inline-block bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest text-xs px-6 py-3 transition">
                    ...
                    return (
                      <div key={req.id} className="p-6 hover:bg-white/[0.02] transition">
                        ...
                        {deal && (
                          <div className="flex items-center gap-3">
                            {deal.status === 'pending' && (
                              <Link href={`/deal?requestId=${req.id}`} className="text-[10px] font-black uppercase tracking-widest bg-red-600 hover:bg-red-700 text-white px-4 py-2 transition">
                                Complete payment →
                              </Link>
                            )}
                            {(deal.status === 'paid' || deal.status === 'completed') && deal.full_beat_url && (
                              <a href={deal.full_beat_url} download className="text-[10px] font-black uppercase tracking-widest bg-white text-black hover:bg-red-600 hover:text-white px-4 py-2 transition">
                                Download beat
                              </a>
                            )}
                            {(deal.status === 'paid' || deal.status === 'completed') && deal.license_url && (
                              <a href={deal.license_url} download className="text-[10px] font-mono uppercase tracking-widest border border-white/20 hover:border-white/40 text-white/50 hover:text-white px-4 py-2 transition">
                                Download license
                              </a>
                            )}
          ...
          <div className="space-y-16">
            ...
            <div className="grid grid-cols-3 border border-white/10">
              {[
                ...
                { num: deals.filter(d => d.status === 'paid' || d.status === 'completed').length, label: 'Beats sold', color: 'text-white' },
            ...
            <div className="border border-white/10 p-8 flex items-center justify-between">
              <div>
                ...
                {profile.stripe_onboarded ? (
                  <p className="font-black uppercase tracking-tight text-green-400">Connected — you receive 80% of each sale</p>
                ) : (
            ...
            <div>
              ...
              {snippets.length === 0 ? (
                <div className="border border-white/5 p-12 text-center">
                  ...
                  <Link href="/browse" className="inline-block bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-widest text-xs px-6 py-3 transition">
                    Browse pool →
                  </Link>
                </div>
              ) : (
                    ...
                    <div key={deal.id} className="p-6 hover:bg-white/[0.02] transition">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          ...
                          <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-white/30 mt-1">
                            Artist: {deal.profiles?.username} · You earn: ${Math.round(deal.agreed_price * 0.80)}
                          </p>
                        </div>
                        <span className={`text-[10px] font-mono uppercase tracking-widest border px-3 py-1 ${
                      ...
                      {deal.status === 'paid' && !deal.full_beat_url && (
                        <div>
                          ...
                          <label className="cursor-pointer inline-block bg-white text-black hover:bg-red-600 hover:text-white font-black uppercase tracking-widest text-xs px-6 py-3 transition">
                            Upload full beat
                            <input type="file" accept="audio/*" className="hidden" onChange={(e) => handleUploadFullBeat(deal.id, e)} />
                          </label>
                        </div>
                      )}
                      {deal.full_beat_url && (
                        <p className="text-[10px] font-mono uppercase tracking-widest text-green-400">✓ Full beat uploaded</p>
                      )}
                    </div>
                  ))}
                </div>
